...
import { IconRotateClockwise, IconCrop, IconX, IconCheck, IconLoader2 } from '@tabler/icons-react';
...
/**
 * Modal pro editaci obrázků - crop a rotace
 */
const ImageEditor = ({ file, isOpen, onClose, onSave }) => {
    ...
    // saveMode se určí automaticky podle operací
    const [imageScale, setImageScale] = useState(1);
    const [activeHandle, setActiveHandle] = useState(null);
    const [isDragging, setIsDragging] = useState(false);
    const [dragStart, setDragStart] = useState(null);
    const [previewCrop, setPreviewCrop] = useState(null); // Pro vizuální oříznutí
    const [isEditingCrop, setIsEditingCrop] = useState(false); // Zda upravujeme existující crop

    // Načíst obrázek při otevření modalu
    useEffect(() => {
        if (isOpen && file) {
            log.info('ImageEditor: Načítám obrázek', { fileName: file.fileName });
            const img = new Image();
            img.crossOrigin = 'anonymous';
            img.onload = () => {
                setOriginalImage(img);
                log.info('ImageEditor: Obrázek úspěšně načten', { 
                    width: img.width, 
                    height: img.height 
                });
            };
            img.onerror = () => {
                log.error('ImageEditor: Nepodařilo se načíst obrázek', { src: file.url || file.preview });
            };
    ...
    }, [originalImage, previewCrop, rotation, cropArea, isCropping]);

    // Překreslení canvasu s aplikovanými změnami
    const drawCanvas = (img = originalImage, currentRotation = rotation) => {
        ...
        // Pokud máme preview crop, zobrazit jen oříznutou část
        if (previewCrop) {
            // previewCrop obsahuje původní souřadnice v obrázku
            const { x: sourceX, y: sourceY, width: sourceWidth, height: sourceHeight } = previewCrop;
            
            // Škálovat oříznutou část pro fit do modalu (s ohledem na rotaci)
        ...
        // Uložit škálování pro crop výpočty
        setImageScale(scale);
        ...
        // Nakreslit crop oblast pokud aktivní
        if (cropArea && isCropping) {
            ...
            // Přidat tmavý overlay mimo výběr
    ...
    const handleRotate = (degrees) => {
        ...
        log.info('ImageEditor: Rotace aplikována', { degrees, newRotation });
        
        // Přidat operaci
        const rotateOp = { type: 'rotate', degrees };
        setOperations(prev => [...prev, rotateOp]);
        
        // useEffect automaticky překreslí
    };
    ...
    const handleCropStart = () => {
        if (previewCrop && !isEditingCrop) {
            ...
            setTimeout(() => {
                ...
                log.info('ImageEditor: Převádím ořez na souřadnice canvas', {
    ...
    const handlePointerDown = (e) => {
        ...
        // Začít nový výběr
        log.info('ImageEditor: Začátek ořezu', { x, y, scale: imageScale });
    ...
    const applyCrop = () => {
        ...
        log.info('ImageEditor: Ořez aplikován', { canvasCrop: cropArea, originalCrop, scale: imageScale });
        ...
        // PAK nastavit preview - useEffect to automaticky překreslí
        setPreviewCrop(originalCrop);
    };

    // Automatické určení saveMode podle operací
    const getSaveMode = () => {
        return operations.some(op => op.type === 'crop') ? 'overwrite' : 'overwrite';
    };
    ...
    const handleSave = async (mode = 'overwrite') => {
        if (operations.length === 0) {
            log.info('ImageEditor: Žádné operace k uložení');
            onClose();
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                log.info('ImageEditor: Úspěšně uloženo', result);
            ...
            log.error('ImageEditor: Uložení selhalo', error);
    ...
    // Reset změn
    const handleReset = () => {
        log.info('ImageEditor: Resetuji stav editoru');
