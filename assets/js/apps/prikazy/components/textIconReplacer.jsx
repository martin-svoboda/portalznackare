...
// Vlastní SVG ikony
const BusIcon = ({ size = 10 }) => (
    <svg width={Math.round(size * 1.7)} height={size} viewBox="0 0 30 16" version="1.1" xmlns="http://www.w3.org/2000/svg">
 ...
 */
export const replaceTextWithIcons = (text, iconSize = 10, hideIcon = false) => {
    if (!text) return text;
                ...
                if (part.startsWith('&')) {
                    ...
                    return iconKeys.map((iconKey, j) => {
                        ...
                        if (iconFactory) {
                            ...
                            return <span key={`${i}-${j}`}>{iconKey}</span>;
                        }
                    });
                }
